...
**Mata Kuliah:** Integrasi Aplikasi Enterprise (BBK2HAB3) [cite: 26]  
...
(1.1 Validasi X-IAE-KEY) — Service memverifikasi bahwa request berasal dari klien yang sah menggunakan API key lokal. Jika tidak valid, request langsung ditolak dengan 401 Unauthorized.
...
(1.2.1.1.1 POST /soap/v1/audit) — Service mengirim XML Envelope berisi data tiket (TicketCreated) ke endpoint audit dosen menggunakan Bearer token M2M. Field <iae:LogContent> memuat detail tiket dalam format JSON (dibungkus CDATA).
...
[cite_start]**Indikator:** Aplikasi sukses menangkap payload JWT dari Cloud Dosen dan berhasil memetakan user ke tabel roles lokal.  
  

* [cite_start]Middleware `VerifySSO.php` berhasil menangkap JWT dari header `Authorization: Bearer`.
* [cite_start]JWT diverifikasi menggunakan public key (JWKS) dari `GET /api/v1/auth/jwks` dengan algoritma RS256.
* Payload JWT (email, nama) dipetakan ke tabel `users` lokal menggunakan `firstOrCreate`, dengan role default `tenant`.
* [cite_start]Untuk transaksi kritis (SOAP & RabbitMQ), aplikasi menggunakan token M2M (`SsoM2MService.php`) dengan `IAE_API_KEY=KEY-MHS-280`, token di-cache 55 menit untuk efisiensi.
...
Indikator: Kode SOAP Client berhasil melakukan transformasi data JSON ke format XML Envelope kaku dan menyimpan ReceiptNumber dari Dosen.  
TercapaiSoapAuditService.php mengubah data tiket (JSON) menjadi SOAP XML Envelope sesuai skema wajib (TeamID, ActivityName, LogContent dengan pembungkus CDATA).  Request dikirim ke endpoint POST /soap/v1/audit dengan menggunakan Bearer Token M2M.  Response berupa XML diparsing menggunakan simplexml_load_string dan XPath untuk mengambil data ReceiptNumber.  ReceiptNumber disimpan secara lokal ke kolom soap_receipt pada tabel database tickets.  
Bukti Log:Plaintext[SOAP] Audit berhasil {"receipt":"IAE-LOG-2026-2C9C20CB"}
Response Endpoint (Potongan):JSON{
    "soap_receipt": "IAE-LOG-2026-2C9C20CB"
}

Modul 3: AMQP Publisher 
Indikator: Aplikasi berhasil mengirimkan event notification dalam bentuk JSON ke RabbitMQ Dosen tanpa memicu error.
 TercapaiRabbitMQService.php mem-publish event ticket.created menuju endpoint POST /api/v1/messages/publish.  Payload disesuaikan secara presisi dengan struktur yang diminta server pusat (routing_key + message).  Event berhasil tampil di Papan Pengumuman RabbitMQ (/board) dengan label Dari: TEAM-08.
 
 Bukti Log:Plaintext[RabbitMQ] Publish berhasil {"event":"ticket.created"}
